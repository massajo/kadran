...
| KDN-124 | Observabilité backend : logs JSON structurés sans PII, MDC, `/actuator/prometheus` sur port de management séparé, conventions de nommage des métriques. **Déplacer Actuator sur le port de management casse le `HEALTHCHECK` de `backend/Dockerfile` (KDN-6), qui interroge le port applicatif : le corriger dans la même PR** | `backend` |
| KDN-125 | Prometheus + Grafana dans `docker/compose.yml`, tableaux de bord versionnés dans `docker/grafana/` | `infra` |
