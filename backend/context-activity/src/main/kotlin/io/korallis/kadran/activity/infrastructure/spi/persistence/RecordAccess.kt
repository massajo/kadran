 ...
 * base porte un fuseau. Le fuseau d'exploitation (spec §4.3) s'applique à la lecture métier —
 * `BusinessDayPolicy`, KDN-33 — jamais au stockage.
 *
 * Doublon délibéré de `identity.infrastructure.spi.persistence.RecordAccess` (KDN-27) : les
 * deux fonctions sont `internal`, donc invisibles d'un module Gradle à l'autre, et il n'existe
 * pas encore d'utilitaire partagé dans `platform` pour ce genre de conversion.
 */
 ...
 * les colonnes d'un `SELECT *` sont typées d'après les métadonnées du `ResultSet`. Passer le
 * type attendu force la conversion documentée de jOOQ.
 */
